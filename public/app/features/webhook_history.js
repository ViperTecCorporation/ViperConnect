...
export function renderWebhookHistory(snapshots, loading, error) {
    ...
    ${snapshots.map(snapshot => `<form class="stack section" data-form="restore-webhook-history">
      ...
      <p>${escapeHtml(snapshot.archived_at)} · ${escapeHtml(snapshot.server)} · ${escapeHtml({ updated: 'Alteração de configuração', removed: 'Remoção da sessão', restored: 'Antes de uma restauração' }[snapshot.reason] || snapshot.reason)}</p>
