...
import { historyControls, historyTable } from './voip.js'
import { scopedHistoryItems } from '../domain/voip_history.js'

// UI checks complement server authorization. Never infer ownership from company
// membership, extension names, username similarity, or arbitrary resource IDs.
export const scopedExtensions = (state: VoipBootstrap) => {
  if (!state.capabilities?.automaticExtensions) return []
...
export const scopedRegistrations = (state: VoipBootstrap) => {
  if (!state.capabilities?.extensionRegistrations) return []
  const ids = new Set(scopedExtensions(state).map(extension => extension.id))
  return [
    ...(state.registrations?.webrtc || []).map((row): Record<string, any> & { transport: string } => ({ ...row, transport: 'webrtc' })),
    ...(state.registrations?.sipRtp || []).map((row): Record<string, any> & { transport: string } => ({ ...row, transport: 'sip_rtp' })),
  ].filter(row => typeof row.id === 'string' && ids.has(row.id) && typeof row.registrationId === 'string' && row.registrationId)
}

export const canDisconnectScopedRegistration = (state: VoipBootstrap): boolean =>
  state.capabilities?.extensionRegistrations === true && state.capabilities.disconnectRegistration !== false

const renderAutomaticResources = (state: VoipBootstrap): string => {
...
/** Fail closed before bootstrap capabilities arrive; never render global resources. */
export const renderScopedVoip = (state: VoipBootstrap, loading: boolean, error: string, urls: Record<string, string> = {}, phones = (state.zapoLines || []).map(line => line.session)): string => `
  ...
  ${state.calls.map(call => `<tr><td>${e(call.callId)}</td><td>${e(call.session)}</td><td>${e(call.direction)}</td><td>${e(call.callerName || call.callerPn || call.peerJid || '—')}</td><td><div class="row-actions">
    ${state.capabilities?.callCommands ? ([['accept', 'Aceitar'], ['reject', 'Rejeitar'], ['end', 'Encerrar'], ['mute', 'Silenciar'], ['unmute', 'Ativar áudio']] as const)
