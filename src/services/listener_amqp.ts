...
import { UNOAPI_EXCHANGE_BRIDGE_NAME, UNOAPI_EXCHANGE_BROKER_NAME, UNOAPI_QUEUE_HISTORY, UNOAPI_QUEUE_LISTENER, UNOAPI_SERVER_NAME, UNOAPI_WORKER_ENGINE } from '../defaults'
...
export class ListenerAmqp implements Listener {
  ...
  public async process(phone: string, messages: object[], type: eventType) {
    ...
    options.type = type === 'history' ? 'topic' : 'direct'
    ...
    const packed = shouldPack
      ...
      : messages
    await amqpPublish(
      type === 'history' ? UNOAPI_EXCHANGE_BROKER_NAME : UNOAPI_EXCHANGE_BRIDGE_NAME,
      providerQueueName(type === 'history' ? UNOAPI_QUEUE_HISTORY : UNOAPI_QUEUE_LISTENER, UNOAPI_SERVER_NAME, this.workerEngine),
      phone,
